...
- 202603261233 github push blocked -> active gh account viewerPermission on shaburyaan/SovAuto is READ, real push returned 403 and repo metadata update cannot complete until write-capable auth is used
- 202603261236 pushAuto fail-fast ok -> command now checks viewerPermission before git add/commit and aborts immediately with explicit write-access error on READ-only account
- 202603261305 pushAuto push path fixed -> pushAuto now mirrors pushSov flow with git add -A, current branch detection and git push origin <branch>; gh metadata update became non-blocking post-step
- 202603261306 push dry-run ok -> git push --dry-run origin main succeeded against shaburyaan/SovAuto while gh viewerPermission stayed READ, confirming the old failure was in pushAuto auth logic rather than git push itself
